...
const timeFunc = require('./timeFunctions.js'); // 시간 표시
// express 객체 생성
const app = express();
// http server 생성
...
io.on('connection', (socket) => { // socket 연결
    ...
    socket.on('join_room', async (room_name, sender_nick) => { // room에 join
        ...
        console.log("Sender nick is " + sender_nick);
        console.log("Joined in " + room_name);
    ...
    socket.on('send_msg', (m_info) => { // socket에서 msg를 보냄
        ...
        new_msg.save((err) => { // Chat msgs Collection에 저장 
            ...
            io.to(socket.room).emit('receive_msg', new_msg); // 같은 room에 있는 모든 socket에게 이벤트 emit (여기서는 1대1 채팅이므로 상대에게만 emit)
        });
    });
    ss(socket).on('sendFile', (stream, dataName, dataSize, m_info) => {
        console.log("sendFile event is occured.\n" + dataName + "/ " + dataSize + 'Byte');
            ///////////////////////////// 일단 여기까지
        var extpoint = (dataName.length-1) - dataName.split("").reverse().join("").indexOf('.');
        var file_name = dataName.substr(0, extpoint); // 파일명
        var file_ext = dataName.substr(extpoint); // 파일 확장자
        var msg_sentTime = new Date(m_info.msg_time);
        var s3Filename = timeFunc.s3_dateTime(msg_sentTime) + '_' + md5(file_name)
            + file_ext;
